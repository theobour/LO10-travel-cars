...
<html>
    ...
    <body>
        ...
        <div class="container">
            ...
            <?php echo("$aeroport_choisi"); ?>
            
            <!-- Affichage de la liste de places de parking dispos -->
            
            <?php $reponse = $bdd->query('SELECT lieu, aeroport, nb_places, prix, adresse FROM site WHERE aeroport =\''. $aeroport.'\' AND nb_places > 0');

                                        $nombre_reservations = 0;
                                        while ($donnees = $reponse->fetch())
                                        {
                                                echo("<div id=liste_reservation>");
                                                echo ("<strong>Aéroport choisi :</strong> $donnees[aeroport] <br/>");
                                                echo ("<strong>Site proposé :</strong> $donnees[lieu] <br/>");
                                                echo ("<strong>Adresse :</strong> $donnees[adresse] <br/>");
                                                echo ("<strong>Nombre de places restantes :</strong> $donnees[nb_places] <br/>");
                                                echo ("<strong>Prix par jour :</strong> $donnees[prix] €<br/>");
                                                
                                                $nombre_reservations++;
                                                
                                                echo("<br/>");
                                                echo ("<form action=check_reservation.php method=post>
                                                          <input type=hidden name=aeroport_choisi value=\"$donnees[aeroport]\" >
                                                          <input type=hidden name=lieu_choisi value=\"$donnees[lieu]\" >
                                                          <input type=hidden name=date_entree value=$date_entree_res >
                                                          <input type=hidden name=date_sortie value=$date_sortie_res >
                                                          <input type=hidden name=prix_choisi value=$donnees[prix] >
                                        ...
                                        if ($nombre_reservations == 0) {echo("Aucune place de parking n'est disponible à l'aéroport $aeroport du $date_entree_loc au $date_sortie_loc.<br/>
